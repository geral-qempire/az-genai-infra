...
## 3. Inputs
Refer to `variables.tf` for the complete list of inputs including AI Services, Storage Account, SQL Server/Database, alert toggles, Action Group config, and optional RBAC for AD groups and Service Principals.

## 4. Outputs
Refer to `output.tf` for the list of outputs including resource IDs and names for AI Services, Storage Account, SQL Server, SQL Database, the Services→Project connection, and any role assignment IDs created.
...
| `modules/ai-services` | Azure AI Services with MI + PE and integrated alerts. | ais-v0.1.0 |
| `modules/storage-account` | Storage Account with Blob/File PEs and integrated alerts. | st-v0.1.0 |
| `modules/sql-server` | Azure SQL logical server with PE. | sqlsrv-v0.1.0 |
| `modules/sql-database` | Azure SQL Database with backup config and integrated alerts. | sqldb-v0.1.0 |
