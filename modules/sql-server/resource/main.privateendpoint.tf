...
locals {
  private_endpoint_name           = lower("pep-sql-${local.region_abbreviation}-${var.service_prefix}-${var.environment}")
  private_endpoint_nic_name       = lower("nic-pep-sql-${local.region_abbreviation}-${var.service_prefix}-${var.environment}")
