variable "name" {
  description = "(Optional) Name of the metric alert. If not provided, defaults to alrt-ttft-<resource-name>."
  type        = string
  default     = null
}
...
For AzureOpenAINormalizedTTFTInMS, pass one or more Azure AI Services (Cognitive Services) account resource IDs (Microsoft.CognitiveServices/accounts).
DESC
  type = list(string)
...
variable "threshold" {
  description = <<DESC
(Optional) Time To First Token (normalized) threshold in milliseconds.
