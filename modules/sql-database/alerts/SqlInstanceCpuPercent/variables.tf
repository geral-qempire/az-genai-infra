variable "name" {
  description = "(Optional) Name of the metric alert. If not provided, defaults to alrt-sqlcpu-<resource-name>."
  type        = string
  default     = null
}
...
For SQL Instance CPU Percent, pass one or more SQL Database resource IDs (Microsoft.Sql/servers/databases).
DESC
  type = list(string)
