variable "name" {
  description = "(Optional) Name of the metric alert. If not provided, defaults to alrt-sqlmem-<resource-name>."
  type        = string
  default     = null
}
...
For SQL Instance Memory Percent, pass one or more SQL Database resource IDs (Microsoft.Sql/servers/databases).
DESC
  type = list(string)
