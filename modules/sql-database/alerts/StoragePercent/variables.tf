variable "name" {
  description = "(Optional) Name of the metric alert. If not provided, defaults to alrt-stor-<resource-name>."
  type        = string
  default     = null
}
...
For SQL Database Storage Percent, pass one or more SQL Database resource IDs (Microsoft.Sql/servers/databases).
DESC
  type = list(string)
...
variable "threshold" {
  description = <<DESC
(Optional) Storage percent threshold.
