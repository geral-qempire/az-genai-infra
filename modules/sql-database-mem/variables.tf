variable "name" {
  description = "(Optional) Name of the metric alert. If not provided, defaults to alrt-mem-<resource-name>."
  type        = string
  default     = null
}
...
For SQL Database App Memory Percent, pass one or more SQL Database resource IDs (Microsoft.Sql/servers/databases).
DESC
  type = list(string)
...
variable "threshold" {
  description = <<DESC
(Optional) Memory percent threshold.
