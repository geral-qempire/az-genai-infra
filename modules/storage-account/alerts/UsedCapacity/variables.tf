variable "name" {
  description = "(Optional) Name of the metric alert. If not provided, defaults to alrt-used-<resource-name>."
  type        = string
  default     = null
}
...
For UsedCapacity, pass one or more Storage Account resource IDs (Microsoft.Storage/storageAccounts).
DESC
  type = list(string)
...
variable "threshold" {
  description = <<DESC
(Optional) Used capacity threshold in bytes.
...
DESC
  ...
  validation {
    ...
    error_message = "threshold must be greater than or equal to 0 bytes."
  }
}
