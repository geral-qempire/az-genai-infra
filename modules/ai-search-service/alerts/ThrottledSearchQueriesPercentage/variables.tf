variable "name" {
  description = "(Optional) Name of the metric alert. If not provided, defaults to alrt-thrpct-<resource-name>."
  type        = string
  default     = null
}
...
For ThrottledSearchQueriesPercentage, pass one or more Azure AI Search Service resource IDs (Microsoft.Search/searchServices).
DESC
  type = list(string)
...
variable "threshold" {
  description = <<DESC
(Optional) Percentage threshold for ThrottledSearchQueriesPercentage.
...
DESC
  ...
  validation {
    ...
    error_message = "threshold must be greater than or equal to 0 percent."
  }
}
